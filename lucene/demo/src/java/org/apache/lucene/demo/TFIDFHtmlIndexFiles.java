...
public class TFIDFHtmlIndexFiles {
    ...
    Date start = new Date();
    //Use standard analyzers to create tokens from the result of parser
    try {
  ...
  static void indexDoc(IndexWriter writer, Path file, long lastModified) throws IOException {
    try (InputStream stream = Files.newInputStream(file)) {
      ...
      TrecContentSource treSrc = new TrecContentSource();
      DocData docData = new DocData();
      try{
          docData = parser.parse(docData, "", date, reader, treSrc);
      } catch (IOException e){
          e.printStackTrace();
      }
      ...
      doc.add(new TextField("content", docData.getTitle() + "\n" + docData.getBody(), Field.Store.YES));
